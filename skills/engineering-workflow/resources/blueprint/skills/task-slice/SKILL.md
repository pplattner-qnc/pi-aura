---
name: task-slice
description: anwalt.de engineering-workflow skill. Breaks an existing large top-level item (high effort, ≈8–13 SP) or a PRD/master plan into vertical, individually human-testable subtask slices. First decides whether slicing is warranted at all — for small/clearly scoped tickets (≈≤3 SP, single vertical scope), recommends implementing directly and leaves a brief decision note in the parent task file instead of creating sub-tasks. When slicing is appropriate, cuts deliberately vertically (backend + frontend per slice), not horizontally (not "all migrations", "all endpoints"), derives the slice dependency graph and groups the slices into parallelisable execution waves, creates one child item per slice in the configured tracker plus a repo Markdown file under `subtasks/`, and adds the plan link to the item's description. Use when the user invokes /task-slice, wants to split an existing large task/PRD/master plan into subtasks, cut vertical slices, derive subtasks from a parent story, group subtasks into parallel waves, break a plan into human-testable increments, or asks whether a ticket needs slicing at all.
---
