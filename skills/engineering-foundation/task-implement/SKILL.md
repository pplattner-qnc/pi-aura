---
name: task-implement
description: anwalt.de engineering-workflow skill. Orchestrates feature-branch implementation for a ticket — both the single (un-sliced) ticket and the sliced/multi-wave case. Use when the user invokes /task-implement, says "implement this ticket", "start working on <KEY>", "build the feature branch", or — for sliced plans — "start/implement the next wave", "do the next batch of subtasks in parallel", "continue the plan wave by wave", or "what can I test now". Reads the ticket plan; if un-sliced, brings up the branch's app container via `start-app`, isolates the DB before any migration via `fork-db`, implements the scope, and emits a Test-URL + worktree path + what-to-test block. If sliced (a `## Subtasks` table with a `Wave` column), runs one execution wave at a time — each slice in its own worktree off the parent branch, implemented in parallel by subagents, merged back sequentially and tested on the parent, with slices closed to done while the parent stays open for task-finish.
---
