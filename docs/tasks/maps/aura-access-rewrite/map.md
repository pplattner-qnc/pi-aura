...
5. `keyring-rewrite` (feature, **done**) — from-scratch keyring in
   `packages/shared/src/keyring/`: `SecretKey` closed enum, `Keyring`
   interface, `FileKeyring`/`MacosKeyring`/`SecretServiceKeyring` (dbus-next,
   DH + HKDF-SHA256 + AES-128-CBC; real CRUD round-trip vs GNOME Keyring
...
- **Where does the generated client live relative to `packages/shared/`?**
  (blocks `aura-client`.) The `@hey-api` generated REST client is in
  `scripts/src/generated/` (gitignored, regenerated by `make codegen`), but
  `aura-client` lives in `packages/shared/`. A shared package importing a
  sibling workspace's gitignored generated tree inverts the dependency
  direction and makes the shared package non-buildable in isolation. Options:
  (a) move the generated client into `packages/shared/src/generated/` (move
  the codegen target + `@hey-api/openapi-ts` devDep there; scripts imports it
  from the shared package); (b) keep it in `scripts/src/generated/` and have
  `packages/shared/aura-client` import it cross-workspace (accepts the
  inverted dep + non-isolated build); (c) duplicate the codegen into the
  shared package (two generated trees). The grilling assumed the generated
  client is the substrate but didn't settle its location. Needs a decision
  before `aura-client` slicing.
