...
- **Where does the generated client live relative to `packages/shared/`?**
  (RESOLVED — move it into `packages/shared`.) The `@hey-api` generated REST
  client moves from `scripts/src/generated/` to
  `packages/shared/src/generated/`. The codegen setup (`openapi.yaml`,
  `openapi-ts.config.ts`, `@hey-api/openapi-ts` devDep, the `make codegen`
  target) moves with it. `packages/shared` owns the generated client; scripts
  and the extension import it via `@pi-aura/shared/aura-client`. Clean
  dependency direction (shared -> generated), single generated tree, shared
  package buildable in isolation. The existing `scripts/src/generated/`
  becomes dead after `call-site-migration` rewires callers and is removed in
  `clients-cleanup`. The `aura-client` task's first slice now includes the
  codegen move.
