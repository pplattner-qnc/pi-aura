...
### slice 3 — drop-digest-json-and-rework-save

Extracted `saveLastDigest(digest)` helper to `@pi-aura/shared/digest/aura-digest` (saveAction + digest-save both use it). `digest-save` writes `last-digest.json` from `store.getCurrentDigest()` — no `dir` param, no spawn; returns a clear error when there is no current digest. Removed `runAuraDigest`/`resolveAuraDigestScriptPath`/`SpawnResult`/`spawn` from `index.ts` (digest-fetch already in-process from slice 2). Removed the spawn mock from `fetch-save-tools.test.ts` (save tests now use `setCurrentDigest` + assert `last-digest.json`). Confirmed no in-process `~/.pi/aura/digest.json` write remains. Minimal skill-doc (`aura-digest.md`) handoff prose update (details.dir → in-memory). The CLI `saveAction` + shim stay (task 5 deletes them). This is the last slice of in-process-fetch.