...
- **Pre-existing (out of scope):** the committed `skills/aura/dist/aura.mjs`
  + `skills/aura-digest/dist/aura-digest.mjs` are clean (no `bearerClient`
  refs — they were rebuilt during `call-site-migration`). No stale-dist
  housekeeping needed; the verifier's stale-dist note was a misattribution.
